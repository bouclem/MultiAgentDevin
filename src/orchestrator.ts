...
import { PatternBase } from "./patterns/base.js";
...
export class Orchestrator {
  private client: DevinClient;
  private gitWorkspace: GitWorkspaceManager;
  ...
  async execute(config: CollaborationConfig): Promise<CollaborationResult> {
    const session = this.createSession(config);
    return this.executeSession(session);
  }

  async executeSession(session: CollaborationSession): Promise<CollaborationResult> {
    session.status = "running";
    session.updatedAt = Date.now();

    globalEmitter.notify(
      session.id,
      "collaboration_started",
      `Starting collaboration: ${session.pattern} — ${session.task.slice(0, 100)}`,
      { pattern: session.pattern, task: session.task }
    );
    ...
    try {
      ...
      globalEmitter.notify(
        ...
        `Collaboration completed: ${result.summary}`,
  ...
  getStatus(sessionId: string): {
    ...
    agents: Array<{ id: string; role: string; status: string; branchName?: string }>;
    rounds: number;
    currentRound: number;
    branches: Array<{ id: string; name: string; status: string; depth: number }>;
  } | undefined {
  ...
  private createPattern(session: CollaborationSession): PatternBase {
