...
export class ParallelPattern extends PatternBase {
  async execute(session: CollaborationSession): Promise<CollaborationResult> {
    ...
    const coordinatorPrompt = buildAgentPrompt(
      "coordinator",
      `Break down the following task into exactly ${numWorkers} independent subtasks. Each subtask should be completable by a single agent without dependencies on other subtasks.\n\nTASK:\n${session.task}`
    );
    ...
    const result = this.buildResult(
      `Parallel decomposition completed: ${workers.length} workers + 1 reviewer. ${this.extractSummary(reviewer)}`,
      solutions,
      reviewer
    );
  ...
  private extractSubtasks(coordinator: Agent, numWorkers: number): string[] {
...
function session_task_fallback(prompt: string, numWorkers: number): string {
