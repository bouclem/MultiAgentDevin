...
import { buildAgentPrompt, STRUCTURED_OUTPUT_SCHEMA, getRoleDefinition } from "../roles.js";
...
export abstract class PatternBase {
  ...
  constructor(
    ...
    emitter: CollaborationEventEmitter,
  ...
  protected async createDevinSession(
    ...
    const response = await this.client.createSession({
      ...
      structured_output_schema: STRUCTURED_OUTPUT_SCHEMA as Record<string, unknown>,
  ...
  protected async waitForAgents(
    ...
    for (const agent of agents) {
      ...
      agent.status = result.status as Agent["status"];
      ...
      if (result.pull_request) {
        (agent as Agent & { prUrl?: string }).prUrl = result.pull_request.url;
      }
    }
  }
  ...
  protected extractSolutions(agents: Agent[]): AgentSolution[] {
    return agents
      ...
      .map((a) => ({
        ...
        prUrl: (a as Agent & { prUrl?: string }).prUrl,
      }));
  }

  protected extractScore(agent: Agent): number | undefined {
    if (!agent.structuredOutput) return undefined;
    const score = (agent.structuredOutput as { score?: unknown }).score;
    return typeof score === "number" ? score : undefined;
  }

  protected extractSummary(agent: Agent): string {
    if (agent.structuredOutput) {
      const summary = (agent.structuredOutput as { summary?: unknown }).summary;
  ...
  protected setupBinaryTreeBranches(
    session: CollaborationSession,
    numAgents: number
  ): { agents: Array<{ branchId: string; branchName: string; parentBranchId?: string }> } {
    ...
    const assignments: Array<{ branchId: string; branchName: string; parentBranchId?: string }> = [];
    ...
    while (currentLeaves.length < numAgents) {
      const newLeaves: typeof currentLeaves = [];
    ...
    for (const agent of agents) {
      ...
      if (agent.structuredOutput) {
        const output = agent.structuredOutput as {
          ...
          issues_found?: Array<{ severity: string; description: string }>;
        ...
        if (output.issues_found?.length) {
          contexts.push(
            `Issues: ${output.issues_found.map((i) => `[${i.severity}] ${i.description}`).join("; ")}`
          );
        }
