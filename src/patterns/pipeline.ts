...
export class PipelinePattern extends PatternBase {
  async execute(session: CollaborationSession): Promise<CollaborationResult> {
    const roles = this.config.specialistRoles?.length
      ...
      : (["architecture", "worker", "testing", "security"] as AgentRole[]);
    ...
    for (let i = 0; i < roles.length; i++) {
      ...
      if (i === 0) {
        prompt = buildAgentPrompt(
          role,
          `You are stage ${stageNum} of a ${roles.length}-stage pipeline. Complete the following task from your expertise area.\n\nTASK:\n${session.task}`,
          undefined,
        ...
        prompt = buildAgentPrompt(
          role,
          `You are stage ${stageNum} of a ${roles.length}-stage pipeline. The previous stages have produced work that you should build upon.\n\nTASK:\n${session.task}`,
          context,
          this.config.customRolePrompts?.[role]
        );
      }
      ...
      prompt += `\n\n${this.gitWorkspace.getGitInstructions(
        ...
        this.gitWorkspace.getBranch(session.id, rootBranch.id)!
      )}`;
    ...
    const result = this.buildResult(
      `Pipeline completed: ${roles.length} stages. Final output: ${this.extractSummary(lastAgent)}`,
      solutions,
      lastAgent
    );
