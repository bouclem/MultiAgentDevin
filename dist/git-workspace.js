...
export class GitWorkspaceManager {
    trees = new Map();
    // --- Tree Creation --------------------------------------------------------
    createTree(collaborationId, rootBranchName, strategy) {
     ...
     */
    splitBranch(collaborationId, parentBranchId, suffixes) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            throw new Error(`No tree found for collaboration ${collaborationId}`);
        const parent = tree.branches.get(parentBranchId);
        if (!parent)
            throw new Error(`Branch ${parentBranchId} not found`);
     ...
     */
    splitAllLeaves(collaborationId, suffixes) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            throw new Error(`No tree found for collaboration ${collaborationId}`);
        ...
        for (const leaf of leaves) {
            const [childA, childB] = this.splitBranch(collaborationId, leaf.id, suffixes);
     ...
     */
    extendBranch(collaborationId, parentBranchId, suffix) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            throw new Error(`No tree found for collaboration ${collaborationId}`);
        const parent = tree.branches.get(parentBranchId);
     ...
     */
    extendLatestLeaf(collaborationId, suffix) {
        const leaves = this.getActiveLeaves(collaborationId);
        if (leaves.length === 0)
            return undefined;
     ...
     */
    getLinearChain(collaborationId) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            return [];
     ...
     */
    getBranchingStrategy(collaborationId) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            return "binary-tree";
        const root = tree.branches.get(tree.rootId);
        return root?.metadata?.strategy ?? "binary-tree";
    }
    // --- Branch Queries -------------------------------------------------------
    getBranch(collaborationId, branchId) {
        const tree = this.trees.get(collaborationId);
        return tree?.branches.get(branchId);
    }
    getActiveBranches(collaborationId) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            return [];
        return [...tree.branches.values()].filter((b) => b.status === "active");
    }
    getActiveLeaves(collaborationId) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            return [];
        return [...tree.branches.values()].filter((b) => b.status === "active" && b.childrenIds.length === 0);
    }
    getBranchesAtDepth(collaborationId, depth) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            return [];
        return [...tree.branches.values()].filter((b) => b.depth === depth && b.status === "active");
    }
    getBranchPath(collaborationId, branchId) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            return [];
        const path = [];
        let current = tree.branches.get(branchId);
        while (current) {
    ...
    setBranchStatus(collaborationId, branchId, status) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            return;
     ...
     */
    eliminateBranch(collaborationId, branchId) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            return undefined;
        const branch = tree.branches.get(branchId);
     ...
     */
    mergeBranch(collaborationId, branchId) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            return;
        const branch = tree.branches.get(branchId);
     ...
     */
    archiveBranch(collaborationId, branchId) {
        this.setBranchStatus(collaborationId, branchId, "archived");
    }
    // --- Agent Assignment -----------------------------------------------------
    assignAgentToBranch(collaborationId, branchId, agentId, sessionId) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            return;
        const branch = tree.branches.get(branchId);
    ...
    updateBranchCommit(collaborationId, branchId, commitSha, commitMessage) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            return;
        const branch = tree.branches.get(branchId);
    ...
    getTreeAsObject(collaborationId) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            return null;
        const root = tree.branches.get(tree.rootId);
    ...
    buildTreeNode(collaborationId, branch) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            return branch;
        const result = { ...branch };
    ...
    getAllBranches(collaborationId) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            return [];
     ...
     */
    getGitInstructions(branch, parentBranch) {
        const strategy = branch.metadata?.strategy;
        if (!parentBranch) {
...
5. Your branch builds upon the work from ${parentBranch.name}`;
     ...
     */
    getLatestGoodBranch(collaborationId, eliminatedBranchId) {
        const tree = this.trees.get(collaborationId);
        if (!tree)
            return undefined;
        ...
        const sameDepthActive = this.getBranchesAtDepth(collaborationId, eliminatedBranch.depth);
