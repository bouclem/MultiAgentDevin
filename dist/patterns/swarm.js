...
export class SwarmPattern extends PatternBase {
    ...
    async execute(session) {
        ...
        const coordinatorPrompt = buildAgentPrompt("coordinator", `Analyze the following task and break it into ${initialAgents} initial subtasks. For each subtask, indicate whether it is "simple" (can be solved by one agent) or "complex" (may need further decomposition).\n\nTASK:\n${session.task}\n\nProvide the subtasks in your structured output as an array of objects with "subtask", "complexity" ("simple" or "complex"), and "suggested_role" fields.`);
        ...
        for (let i = 0; i < subtasks.length; i++) {
            ...
            const agent = this.createAgent(subtask.role, `Swarm Agent ${i + 1}`, prompt, branchId, branchName);
        ...
        await Promise.all(initialSwarmAgents.map((sa) => this.createDevinSession(session, sa.agent)));
        ...
        while (currentDepth < this.maxDepth) {
            ...
            const completedAgents = [...this.swarm.values()].filter((sa) => sa.status === "working" && sa.agent.status === "finished");
            ...
            for (const parentSA of completedAgents) {
                ...
                if (needsExpansion && totalAgents < this.maxTotalAgents && parentSA.depth < this.maxDepth - 1) {
                    ...
                    for (let j = 0; j < 2; j++) {
                        ...
                        let prompt = buildAgentPrompt("worker", `You are a dynamically spawned sub-agent. Your parent agent identified that more work is needed.\n\nPARENT CONTEXT:\n${this.buildContextFromAgents([parentSA.agent])}\n\nYOUR SUBTASK:\n${subtask}`);
                        prompt += `\n\n${this.gitWorkspace.getGitInstructions(childBranch, parentBranch)}`;
                        const childAgent = this.createAgent("worker", `${parentSA.agent.roleName} → Sub ${j + 1}`, prompt, childBranch.id, childBranch.name, parentBranch.id);
            ...
            if (newSwarmAgents.length > 0) {
                ...
                await Promise.all(newSwarmAgents.map((sa) => this.createDevinSession(session, sa.agent)));
        ...
        const aggregatorPrompt = buildAgentPrompt("coordinator", `The swarm has completed. Aggregate all agent outputs into a final unified solution.\n\nORIGINAL TASK:\n${session.task}\n\nALL AGENT SOLUTIONS (${allCompletedAgents.length} agents):\n${context}\n\nProduce a final merged solution that combines the best work from all agents.`);
        ...
        const result = this.buildResult(`Swarm completed: ${this.swarm.size} agents across ${session.rounds.length} depth levels. ${this.extractSummary(aggregator)}`, solutions, aggregator);
    ...
    // --- Swarm Helpers --------------------------------------------------------
    extractSwarmSubtasks(coordinator, count) {
    ...
    checkNeedsExpansion(agent) {
        if (!agent.structuredOutput)
            return false;
        const output = agent.structuredOutput;
        if (output.needs_further_work === true)
            return true;
        if (output.remaining_subtasks && output.remaining_subtasks.length > 0)
            return true;
        if (output.status === "blocked")
            return true;
        return false;
    }
    deriveChildSubtasks(parentSA) {
        ...
        const output = parentSA.agent.structuredOutput;
        const subtasks = [];
