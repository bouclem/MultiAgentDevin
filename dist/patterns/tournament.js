...
export class TournamentPattern extends PatternBase {
    async execute(session) {
        ...
        for (let i = 0; i < initialAgents; i++) {
            let prompt = buildAgentPrompt("worker", `You are agent ${i + 1} of ${initialAgents} in a tournament. Solve the following task as well as you can. You are competing against other agents — produce the best possible solution.\n\nTASK:\n${session.task}`);
            prompt += `\n\n${this.gitWorkspace.getGitInstructions(rootBranch)}`;
            const agent = this.createAgent("worker", `Agent ${i + 1}`, prompt, rootBranch.id, rootBranch.name);
        ...
        for (let round = 1; round <= numRounds; round++) {
            ...
            const reviewerPrompt = buildAgentPrompt("reviewer", `You are the judge of a tournament round. Score each of the following ${activeAgents.length} solutions on a scale of 0-100. Then rank them from best to worst. Identify which solutions should be eliminated (the bottom half).\n\nTASK:\n${session.task}\n\nSOLUTIONS:\n${context}\n\nProvide a score for each solution and identify the winners and losers.`);
            ...
            const reviewerOutput = reviewer.structuredOutput;
            ...
            for (const winner of winners) {
                ...
                let winnerPromptA = buildAgentPrompt("worker", `You WON round ${round} with a score of ${winner.score}/100. Continue improving your solution. Build upon your previous work.\n\nTASK:\n${session.task}`, this.buildContextFromAgents([winner.agent]));
                winnerPromptA += `\n\n${this.gitWorkspace.getGitInstructions(childA, winnerBranch)}`;
                const agentA = this.createAgent("worker", `${winner.agent.roleName} → Branch A`, winnerPromptA, childA.id, childA.name, winnerBranch.id);
                ...
                if (recycle && losers.length > 0) {
                    ...
                    let recycledPrompt = buildAgentPrompt("worker", `You were ELIMINATED in round ${round} with a score of ${loser.score}/100. You are now being RECYCLED to work on a winning branch. Study the winning solution and help improve it. You can see the winning agent's work in the branch history.\n\nTASK:\n${session.task}`, this.buildContextFromAgents([winner.agent]));
                    recycledPrompt += `\n\n${this.gitWorkspace.getGitInstructions(childB, winnerBranch)}`;
                    agentB = this.createAgent("worker", `${loser.agent.roleName} → Recycled to ${childB.name}`, recycledPrompt, childB.id, childB.name, winnerBranch.id);
                    ...
                    let winnerPromptB = buildAgentPrompt("worker", `You WON round ${round}. Now try an ALTERNATIVE approach to the same task. Explore a different strategy.\n\nTASK:\n${session.task}`, this.buildContextFromAgents([winner.agent]));
                    winnerPromptB += `\n\n${this.gitWorkspace.getGitInstructions(childB, winnerBranch)}`;
                    agentB = this.createAgent("worker", `${winner.agent.roleName} → Branch B (alt)`, winnerPromptB, childB.id, childB.name, winnerBranch.id);
                }
        ...
        const bestAgent = activeAgents.length > 0
            ...
            : undefined;
        const result = this.buildResult(`Tournament completed in ${session.rounds.length} rounds. Champion: ${bestAgent?.roleName ?? "N/A"} (score: ${this.extractScore(bestAgent) ?? "N/A"}). ${bestAgent ? this.extractSummary(bestAgent) : ""}`, finalSolutions, bestAgent);
