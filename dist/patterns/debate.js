...
export class DebatePattern extends PatternBase {
    async execute(session) {
        ...
        for (let i = 0; i < numAgents; i++) {
            ...
            let prompt = buildAgentPrompt(role, `Solve the following task from your area of expertise. Provide a complete solution.\n\nTASK:\n${session.task}`, undefined, this.config.customRolePrompts?.[role]);
            ...
            const agent = this.createAgent(role, roleName, prompt, branchInfo?.branchId, branchInfo?.branchName, branchInfo?.parentBranchId);
            if (branchInfo) {
        ...
        const result = this.buildResult(`Debate completed: ${solvers.length} specialists + 1 reviewer. ${this.extractSummary(reviewer)}`, solutions, reviewer);
