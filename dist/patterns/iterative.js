...
export class IterativePattern extends PatternBase {
    async execute(session) {
        ...
        for (let round = 0; round < maxRounds; round++) {
            ...
            let workerPrompt;
            if (round === 0) {
                workerPrompt = buildAgentPrompt("worker", `Implement the following task completely. Write production-ready code.\n\nTASK:\n${session.task}`);
            }
            else {
                workerPrompt = buildAgentPrompt("worker", `Improve your previous solution based on the reviewer's feedback. Address all issues identified.\n\nTASK:\n${session.task}`, lastContext);
            }
            ...
            workerPrompt += `\n\n${this.gitWorkspace.getGitInstructions(this.gitWorkspace.getBranch(session.id, branchId), round > 0 ? this.gitWorkspace.getBranch(session.id, rootBranch.id) : undefined)}`;
        ...
        const result = this.buildResult(`Iterative refinement completed in ${session.rounds.length} rounds. Final score: ${lastScore}/100. ${finalAgent ? this.extractSummary(finalAgent) : "No output"}`, solutions, finalAgent ?? undefined);
